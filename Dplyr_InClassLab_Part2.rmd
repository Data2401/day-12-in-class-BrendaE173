...
```{r}
dim(my_flights) #dimension
colnames(my_flights) # column names
glimpse(my_flights)
??flights
...
my_flights <- my_flights %>% mutate(gain= dep_delay - arr_delay, speed = distance/ air_time * 60, before = 1)
...
my_flights <- my_flights %>%
  mutate(gain = dep_delay - arr_delay, speed = distance/ air_time * 60, before = 1) %>% arrange(desc(gain))


```

```{r}
tally(group_by(my_flights, origin))
...
my_flights %>%
  group_by(dest) %>%
  summarize(n=n(), 
            n_airtime= n_distinct(air_time),
            n_distance = n_distinct(distance))
...
```{r}
houston_flights <- my_flights %>% select(origin, dest, gain) %>% filter(dest %in% c("HOU", "IAG"))
```
...
my_flights %>%
  ...
  summarise(across(air_time,
            list(mean = -mean(., na.rm=T),
                 ...
                 min(desc(air_time), max(desc(air_time))))
...
starwars %>% group_by(homeworld) %>% summarise(n_characters = desc(n()))
...
```{r}
starwars %>% filter(eye_color == "brn") %>% summarise(mean_height = mean(height))
